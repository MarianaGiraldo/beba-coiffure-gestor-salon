...
-- Trigger para actualizar fac_total en FACTURA_SERVICIO después de insertar DETALLE_FACTURA_SERVICIO
CREATE TRIGGER trg_after_insert_detalle_factura
AFTER INSERT ON DETALLE_FACTURA_SERVICIO
FOR EACH ROW
BEGIN
  ...
  SET fac_total = (
    ...
    WHERE dfs.fac_id = NEW.fac_id
  )
  WHERE fac_id = NEW.fac_id;
END;
//

-- Trigger para actualizar fac_total en FACTURA_SERVICIO después de eliminar DETALLE_FACTURA_SERVICIO
CREATE TRIGGER trg_after_delete_detalle_factura
AFTER DELETE ON DETALLE_FACTURA_SERVICIO
FOR EACH ROW
BEGIN
  UPDATE FACTURA_SERVICIO 
  SET fac_total = (
    ...
    FROM DETALLE_FACTURA_SERVICIO dfs
    JOIN SERVICIO s ON dfs.ser_id = s.ser_id
    WHERE dfs.fac_id = OLD.fac_id
  )
  WHERE fac_id = OLD.fac_id;
END;
//

-- Trigger para actualizar fac_total en FACTURA_SERVICIO después de actualizar DETALLE_FACTURA_SERVICIO
CREATE TRIGGER trg_after_update_detalle_factura
AFTER UPDATE ON DETALLE_FACTURA_SERVICIO
FOR EACH ROW
BEGIN
  -- Update the old invoice if it changed
